  ...
  async function loadRecordings() {
    try {
      ...
      const recordingPromises = recordingKeys
        .filter((key: string) => typeof key === 'string' && key.startsWith('recording_'))
        .map(async (key: any) => {
...
<main class="min-h-screen bg-gray-100 text-right p-4 md:p-6 font-kitab">
  ...
  <div class="flex justify-center mb-8 gap-6">
    ...
    >
      <svg viewBox="0 0 100 100" class="w-10 h-10">
        <circle cx="50" cy="50" r="46" fill={isRecording ? "#f87171" : "#ef4444"}></circle>
      </svg>
    </button>
  ...
  <div class="bg-white shadow-lg rounded-lg p-4 mb-8 {!currentRecording ? 'opacity-50 pointer-events-none' : ''}">
    ...
    <div class="flex justify-between items-center mb-2">
      ...
      <div class="text-lg font-medium text-center text-black">{currentRecording.name}</div>
  ...
  {/if}
    ...
    <div class="flex justify-between items-center mb-3">
      ...
      <div class="flex items-center gap-2">
        ...
        >
          {#if isPlaying}
          <svg class="h-8 w-8 "  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round">  <rect x="6" y="4" width="4" height="16" />  <rect x="14" y="4" width="4" height="16" /></svg>
          {:else}
          <svg class="h-8 w-8 "  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round">  <polygon points="5 3 19 12 5 21 5 3" /></svg>
          {/if}
        </button>
        ...
        >
        <svg class="h-8 w-8"  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <path d="M4 12v-3a3 3 0 0 1 3 -3h13m-3 -3l3 3l-3 3" />  <path d="M20 12v3a3 3 0 0 1 -3 3h-13m3 3l-3-3l3-3" /></svg>
        </button>
      </div>
  ...
  <footer class="text-center text-gray-600 text-sm">
    من مشاريع <a href="https://nuqayah.com" target="_blank" class="text-blue-600 hover:underline">نُقاية</a>
  </footer>
</main>
